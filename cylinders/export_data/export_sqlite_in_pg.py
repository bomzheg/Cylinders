...
SQL_CYLINDERS = """
    CREATE TABLE "СоотношениеОбъёмов"  (
        ID SERIAL PRIMARY KEY NOT NULL,
        "Объём баллона, л" numeric(6,3) NOT NULL,
        "Давление, ат" smallint NOT NULL,
        "Объём газа, м3" numeric(8,3) NOT NULL,
        "Температура, °С" smallint NOT NULL,
        "Колличество в связке" smallint NOT NULL,
        "Объём газа в одном баллоне, м3" numeric(6,3) NOT NULL,
...
def get_cylinders_from_sqlite(conn: Connection):
    ...
    """)
    data = cur.fetchall()
    data = list(map(lambda x: [*x[0: 7], bool(x[7])], data))
    cur.close()
    return data
